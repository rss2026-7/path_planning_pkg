...
- **Part A:** Plan trajectories in a known occupancy grid map from the car's current position to a goal pose using
  either a grid-based or sampling-based motion planning method.
...
- **Part C:** Combine the above two goals to enable real-time path planning and execution in the
  simple `racecar_simulator`. For this part, you will also be required to localize the car by running the particle
...
You will have **until Wednesday, April 15th** to complete this lab; you should start early! This lab has multiple
...
**Importantly, we will verify that your path planner can compute a path in less than 30 seconds between two arbitrary points we select.** For reference, the TA code calculates a path between opposite ends of the map in approximately 7 seconds.
...
- Visualization of the planned paths (see Trajectory Utilities) from implemented grid-based or sampling-based planning algorithms
...
the word limit is increased to 3500 instead of the regular 2500.
...
- **Detailed comparison of motion planning algorithms**
    - Explain path planning algorithms, and the strengths and weaknesses of grid-based versus sampling-based methods.
      ...
      functions would you use? How would you ensure the car would not choose paths close to the wall?
        - Specifically, please discuss the following properties of planning algorithms, along with any others you
          ...
          of search after construction. Note that some of these are only applicable to grid-based planners and some
          are only applicable to sampling-based planners.
    - **(Bonus +3 points):** Implement both a grid-based and sampling-based path planning algorithm, and numerically
      compare the strengths and weaknesses of each. Demonstrate your pure pursuit controller on paths generated by ONE
      algorithm.
- As in previous labs, a critical part of the evaluation will be the metrics you choose. We have suggested properties of comparison but we have **not** suggested specific metrics. 
...
- "2D Nav Goal" $\rightarrow$ `/goal_pose`
- "2D Pose Estimate" $\rightarrow$ `/initialpose`
- "Publish Point" $\rightarrow$ `/clicked_point`
...
- Create a simple path planner that will find collision-free paths in the current map. The code is set up to handle
  paths as `geometry_msgs/PoseArray` messages. Once a trajectory is generated, you can visualize the path, start, and goal positions on the `/planned_trajectory/path`, `/planned_trajectory/start_point`, and `/planned_trajectory/end_pose` topics in RViz.
- (OPTIONAL) Implement both a sampling-based and grid-based planning algorithm.
...
The biggest difference between grid-based planning and sampling-based planning is speed and optimality: grid-based
methods can guarantee optimal solutions (with the caveats on optimality we discussed in class), but usually do so at the expense of computation time. A question you should ask yourself is: how would you know if a solution is optimal (or close to optimal)? 
...
and completeness. A common strategy is to use a regular grid to discretize the configuration space. If we then use a search 
...
Grid-based representations and other regular discretizations, while providing some kinds of performance guarantees, are
...
- [Rapidly-exploring Random Trees (RRT, RRT-Connect, and RRT\*)](https://arxiv.org/pdf/1105.1186.pdf)
- [Probabilistic Roadmaps (PRM and PRM\*)](http://www.staff.science.uu.nl/~gerae101/pdf/compare.pdf)

**Remember, for this lab we are requiring you to implement ONLY ONE grid-based algorithm OR sampling-based algorithm.
You can choose which explicit algorithm you want to implement. Optionally, you can choose to implement one of each (one
grid-based AND one sampling-based algorithm), for extra credit.**
...
circles to avoid already explored regions of space. This allows it to quickly explore, at the expense of optimality
guarantees.
...
Search algorithms often tend to cut corners closely since they are attempting to minimize distance or time. Sometimes the
...
Provided basement map (left) and dilated map (right). Disc element, 10px radius.
...
A different possible heuristic is to use the length of the [Dubins curve](https://en.wikipedia.org/wiki/Dubins_path) between pairs of start and end states, which more accurately estimates nonholonomic motion (other
sources: [1](https://pypi.org/project/dubins/)), helping account for orientation. Performing a lower dimensional search such as Dijkstra's algorithm on the grid map can
...
1. Manually define or load a saved path. We have provided a framework to create these with clicks on the RViz map (described below in Trajectory Utilities). See how to do this with the `launch/debug/load_trajectory.launch.xml` file. You can visualize the loaded trajectory path, start point, and end pose on the "/loaded_trajectory/path", "/loaded_trajectory/start_point", "loaded_trajectory/end_pose" topics in RViz.
...
*Trajectory Builder*: Run `ros2 launch path_planning build_trajectory.launch.xml` to build the trajectory. You can build trajectories using the "Publish Point" button in RViz. The `.traj` file will be saved in `~/lab6_trajectories` folder with a
...
*Trajectory Loader*: Run `ros2 launch path_planning load_trajectory.launch.xml` to load the trajectory. Make sure to specify the path to the trajectory in the parameter within `launch/debug/load_trajectory.launch.xml`. This will visualize and publish (to `/loaded_trajectory/path` topic, which you can listen to in pure pursuit). A default trajectory for the Stata basement map has been provided in the `~/lab6_trajectories` folder. Note, there is some uniqueness to this utility in that it publishes exactly once, so make sure you add the visualization topics and save them after running `load_trajectory` the first time, then for all successive runs you should be able to see the visualization.
...
In simulation, you can test with the ground truth odometry (no particle filter) by running

```
ros2 launch path_planning sim_plan_follow.launch.xml
```

Again, this uses the parameters from `config/sim/sim_config`, so double check these if nothing seems to be working! To test the system with your particle filter, run
